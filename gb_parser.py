...
def export_messages_to_file(messages):    
    ...
    with open(f'.\\Export\\messages-{today_str}-q1.txt', 'w', encoding='utf-8') as file:  
        ...
        file.write("Перечисли темы, которые обсуждались.\n")   
        file.write("Какие эмитенты обсуждались ?\n")
        file.write("Какие темы макроэкономики обсуждались ?\n")
        file.write("Какие эмитенты, если такие были, были позитивно оценены и почему? Детально напиши про это.\n")
        file.write("Какие эмитенты, если такие были, были нейтрально оценены и почему? Детально напиши про это.\n")
        ...
        file.write(f'Название ответа должно быть: Анализ информации долгового рынка за {d.strftime("%Y-%m-%d")}. \n\n\n')
        for m in messages:  
            file.write(m + '\n') 
    ...
    with open(f'.\\Export\\messages-{today_str}-q2.txt', 'w', encoding='utf-8') as file:  
        file.write("Ты аналитик по облигациям и анализируешь открытые источники информации и формируешь заключения.\n")
        file.write("Ниже приведены сообщения специализированного ресурса по облигациям. Сделай анализ сообщений. Напиши:\n")
        file.write("Какие проблемы и какие позитивные моменты обсуждались по эмитентам. Перечисли в разрезе каждого из обсждаемого эмитента.\n")
        file.write(f'Название ответа должно быть: Анализ эмитентов долгового рынка за {d.strftime("%Y-%m-%d")}. \n\n\n')
        for m in messages:  
            file.write(m + '\n') 
