...
def export_messages_to_file(messages):    
    ...
    with open(f'.\\Export\\messages-{today_str}-raw.txt', 'w', encoding='utf-8') as file1:  
        for m in messages:  
            file1.write(m + '\n')
                
    with open(f'.\\Export\\messages-{today_str}-q1.txt', 'w', encoding='utf-8') as file:  
        file.write("Ты аналитик по облигациям и анализируешь открытые источники информации и формируешь заключения.\n")
        file.write("Ниже приведены сообщения специализированного ресурса по облигациям. Сделай анализ сообщений. Напиши:\n")
        file.write(f'В ответе назови раздел: Анализ обсуждений долгового рынка за {d.strftime("%Y-%m-%d")}. Главные темы.\n')
        file.write(f'Перечисли темы, которые обсуждались. Темы перечисли в формате списка. Для каждой теме напиши кратко что было по этой теме. Формат ответа: Название темы. Краткое содержание по этой теме. \n')   
        file.write("Какие темы макроэкономики обсуждались ? Темы перечисли в формате списка. Для каждой теме напиши кратко что было по этой теме. Формат ответа: Название темы. Краткое содержание по этой теме.\n")
        file.write("Сделай анализ тональности сообщений. Напиши оценку тональности и настроений сообщений. \n\n\n")
        for m in messages:  
            file.write(m + '\n') 
            
    with open(f'.\\Export\\messages-{today_str}-q2.txt', 'w', encoding='utf-8') as file:  
        file.write("Ты аналитик по облигациям и анализируешь открытые источники информации и формируешь заключения.\n")
        file.write("Ниже приведены сообщения специализированного ресурса по облигациям. Сделай анализ сообщений. Напиши:\n")
        file.write("Какие эмитенты обсуждались. Напиши ответ в таком формате:\n")
        file.write("   Название эмитента\n")
        file.write("     информация по этому эмитенту\n")
        file.write("     	 ...\n")        
        file.write("     	 \n")     
        file.write(f'Название ответа должно быть: Анализ обсуждений эмитентов долгового рынка за {d.strftime("%Y-%m-%d")}. \n\n\n')
        for m in messages:  
            file.write(m + '\n') 
